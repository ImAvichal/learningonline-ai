...
//
// PRIVACY RULE: every customer-facing message has exactly ONE visible recipient.
// Campaigns fan out one message per person. Multi-recipient To/CC is blocked.
...
// Sends ONE private email via Resend. The safety helper throws before any API
// call if callers pass an array with >1 address, a comma-separated list, or a
// semicolon-separated list.
export async function sendCampaignEmail({ apiKey, from, replyTo, to, subject, html, text }) {
  const privateTo = privateRecipientPayload(to)
