...
//   2. localStorage saved preference (explicit picks only: URL param or manual selector)
    ...
    fetch('/api/region')
      ...
      .then(data => {
        if (data?.region && REGIONAL_PRICING[data.region]) {
          ...
          // Geo results DISPLAY but never PERSIST. Only explicit choices
          // (?region= URL param, or a manual selector) are saved — otherwise a
          // first load through a mis-located network/VPN locks the wrong
          // currency in forever, and genuine location changes never register.
          try {
            const saved = localStorage.getItem(REGION_KEY)
            if (!saved) {
