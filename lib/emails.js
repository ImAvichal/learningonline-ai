...
/**
 * Send a "your card will be charged in 3 days" reminder near the end of the
 * Journey 7-day trial. Fires from customer.subscription.trial_will_end.
 * Never throws — callers don't need try/catch.
 */
export async function sendTrialEndingReminder({ to, name, trialEnd }) {
  const apiKey = process.env.RESEND_API_KEY
  const from = process.env.EMAIL_FROM || 'LeO AI <hello@learningonline.ai>'
  const replyTo = process.env.EMAIL_REPLY_TO || 'hello@learningonline.ai'
  ...
  if (!apiKey) {
    console.log('[email] RESEND_API_KEY not configured — skipping trial-reminder send to', to)
    return { sent: false, reason: 'not_configured' }
  }

  const firstName = (name || '').split(' ')[0] || 'there'
  const chargeDate = trialEnd
    ? new Date(trialEnd * 1000).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })
    : 'in a few days'

  const subject = `Your Journey trial ends soon — card charges ${chargeDate}`
  const text = `Hi ${firstName},

Your 7-day free trial of Journey is ending soon. Your card will be charged automatically on ${chargeDate} — a single one-off payment, not a subscription.

Nothing to do if you'd like to keep going — access continues seamlessly.

If you'd rather not continue, you can cancel any time before ${chargeDate} at no charge: ${appUrl}/dashboard/account
...
<body style="margin:0;background:#eef1f7;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1a2138;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#eef1f7;padding:32px 16px;"><tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" role="presentation" style="max-width:560px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(16,24,54,0.08);">
      ...
      <tr><td style="padding:36px 40px 8px;"><h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#05091A;">Your trial ends soon</h1></td></tr>
      <tr><td style="padding:8px 40px;font-size:15px;line-height:1.6;color:#2a3350;">
        <p style="margin:0 0 16px;">Hi ${firstName},</p>
        <p style="margin:0 0 16px;">Your 7-day free trial of <strong>Journey</strong> is ending soon. Your card will be charged automatically on <strong>${chargeDate}</strong> — a single one-off payment, not a subscription.</p>
        <p style="margin:0 0 16px;">Nothing to do if you'd like to keep going — access continues seamlessly.</p>
        <p style="margin:0 0 16px;">If you'd rather not continue, you can cancel any time before ${chargeDate} at no charge.</p>
      </td></tr>
      <tr><td align="center" style="padding:8px 40px 36px;"><a href="${appUrl}/dashboard/account" style="display:inline-block;background:#1A6EFF;color:#fff;text-decoration:none;font-weight:700;font-size:15px;padding:14px 32px;border-radius:10px;">Manage my trial →</a></td></tr>
      <tr><td style="padding:20px 40px;border-top:1px solid #eceff5;"><p style="margin:0;color:#8a93a8;font-size:12px;">From AI anxiety to AI awareness. · <a href="${appUrl}" style="color:#1A6EFF;text-decoration:none;">learningonline.ai</a></p></td></tr>
    </table>
  </td></tr></table>
</body></html>`
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[email] Resend API error (trial reminder):', res.status, await res.text().catch(() => ''))
      return { sent: false, reason: 'api_error', status: res.status }
    }
    return { sent: true }
  } catch (err) {
    console.error('[email] Trial reminder send error:', err.message)
    return { sent: false, reason: 'network_error' }
  }
}

/**
 * Send a "we couldn't charge your card" notice when a Journey trial's day-7/8
 * charge (or a subsequent Smart Retry) fails. Never throws.
 */
export async function sendPaymentFailedEmail({ to, name, amount, currency }) {
  const apiKey = process.env.RESEND_API_KEY
  const from = process.env.EMAIL_FROM || 'LeO AI <hello@learningonline.ai>'
  const replyTo = process.env.EMAIL_REPLY_TO || 'hello@learningonline.ai'
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.learningonline.ai'
  ...
  if (!apiKey) {
    console.log('[email] RESEND_API_KEY not configured — skipping payment-failed send to', to)
    return { sent: false, reason: 'not_configured' }
  }

  const firstName = (name || '').split(' ')[0] || 'there'
  const amountLabel = amount ? formatAmount(amount, currency) : 'your Journey payment'

  const subject = `We couldn't process your payment — action needed`
  const text = `Hi ${firstName},

We tried to charge ${amountLabel} for your Journey access and the payment didn't go through. This usually just means the card details need updating.

Update your payment method here: ${appUrl}/dashboard/account

We'll automatically try again — no action needed if you expect the same card to work, but updating it directly is the fastest fix if anything's changed.
...
<body style="margin:0;background:#eef1f7;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1a2138;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#eef1f7;padding:32px 16px;"><tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" role="presentation" style="max-width:560px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(16,24,54,0.08);">
      <tr><td style="background:#05091A;padding:28px 40px;"><span style="font-weight:800;font-size:20px;color:#fff;letter-spacing:-0.02em;">LeO <span style="color:#3D8BFF;">AI</span></span></td></tr>
      <tr><td style="padding:36px 40px 8px;"><h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#05091A;">Payment didn't go through</h1></td></tr>
      <tr><td style="padding:8px 40px;font-size:15px;line-height:1.6;color:#2a3350;">
        <p style="margin:0 0 16px;">Hi ${firstName},</p>
        <p style="margin:0 0 16px;">We tried to charge <strong>${amountLabel}</strong> for your Journey access and the payment didn't go through. This usually just means the card details need updating.</p>
        <p style="margin:0 0 16px;">We'll automatically try again — but updating your card directly is the fastest fix if anything's changed.</p>
      </td></tr>
      <tr><td align="center" style="padding:8px 40px 36px;"><a href="${appUrl}/dashboard/account" style="display:inline-block;background:#1A6EFF;color:#fff;text-decoration:none;font-weight:700;font-size:15px;padding:14px 32px;border-radius:10px;">Update payment method →</a></td></tr>
      <tr><td style="padding:20px 40px;border-top:1px solid #eceff5;"><p style="margin:0;color:#8a93a8;font-size:12px;">From AI anxiety to AI awareness. · <a href="${appUrl}" style="color:#1A6EFF;text-decoration:none;">learningonline.ai</a></p></td></tr>
    </table>
  </td></tr></table>
</body></html>`
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[email] Resend API error (payment failed):', res.status, await res.text().catch(() => ''))
      return { sent: false, reason: 'api_error', status: res.status }
    }
    return { sent: true }
  } catch (err) {
    console.error('[email] Payment-failed send error:', err.message)
    return { sent: false, reason: 'network_error' }
  }
}
