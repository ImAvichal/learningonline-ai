...
export function AuthProvider({ children }) {
  ...
  const loadUser = async (authUser) => {
    try {
      ...
      const profileRes = await withTimeout(
        supabase.from('users_profile').select('*').eq('id', authUser.id).limit(1)
      )
      const profile = profileRes?.data?.[0] || null

      if (!profile) {
        // Profile fetch failed or genuinely missing — try to create, but tolerate conflict
        const newProfile = {
        ...
        if (upsertErr) console.warn('Profile upsert error (non-fatal):', upsertErr.message)
      ...
      const purchaseRes = await withTimeout(
        supabase.from('purchases').select('tier, payment_status').eq('user_id', authUser.id).in('payment_status', ['paid', 'complete', 'completed']).order('created_at', { ascending: false }).limit(1)
      )
      const latestPurchase = purchaseRes?.data?.[0] || null
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, signup, logout, loginWithGoogle, loginWithLinkedIn, updateUser, markLessonComplete, saveScore, trackDownload, refreshUser, supabase }}>
      {children}
