...
// Wrap a Supabase query with a hard timeout so one slow/blocked query
// can't stall the entire auth flow.
const withTimeout = (promise, ms = 5000, fallback = { data: null }) => {
  return Promise.race([
    promise,
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      // Hard timeout: never let loading hang for >8 seconds
      const timeout = setTimeout(() => {
        console.warn('Auth init taking too long — releasing loading state')
        setLoading(false)
      }, 8000)
      try {
  ...
  const loadUser = async (authUser) => {
    try {
      ...
      const { data: progressData } = await withTimeout(
        supabase.from('course_progress').select('lesson_id').eq('user_id', authUser.id).eq('completed', true)
      )

      // Load scores
      const { data: scoresData } = await withTimeout(
        supabase.from('assessment_scores').select('module_id, score, score_category').eq('user_id', authUser.id)
      )
      ...
      const { data: latestPurchase } = await withTimeout(
        supabase.from('purchases').select('tier, payment_status').eq('user_id', authUser.id).in('payment_status', ['paid', 'complete', 'completed']).order('created_at', { ascending: false }).limit(1).maybeSingle()
      )
  ...
  // Re-fetch user state from server (used after payment to pick up new entitlement)
  ...
  const loginWithGoogle = async (intendedRedirect = null) => {
    // Pass redirect target through the OAuth callback URL itself.
    // localStorage can be wiped by some browsers during OAuth round-trip,
    // so we encode it in the URL as a more reliable fallback.
    const callbackUrl = new URL(`${window.location.origin}/auth/callback`)
    if (intendedRedirect) {
      callbackUrl.searchParams.set('next', intendedRedirect)
      // Also save to localStorage as a secondary fallback
  ...
  // ── claimParentsTier ─────────────────────────────────────────────────────
  // Marks the user as having claimed the free Parents & Caregivers module.
  // Idempotent: only sets selected_tier='parents' if currently null/undefined.
  // Does NOT overwrite paid tiers (journey, pro).
  const claimParentsTier = async () => {
    if (!user?.id || user?.isDevUser) return
    if (user?.tier && user.tier !== 'parents') return  // don't downgrade paid users
    // Optimistic UI
    ...
    try {
      await supabase
        ...
        .eq('id', user.id)
        .is('selected_tier', null)  // only update if currently null — protects paid users
    } catch (err) {
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, signup, logout, loginWithGoogle, loginWithLinkedIn, updateUser, markLessonComplete, claimParentsTier, saveScore, trackDownload, refreshUser, supabase }}>
      {children}
