...
export function AuthProvider({ children }) {
  ...
  const loadUser = async (authUser) => {
    ...
    try {
      ...
      if (!profile) {
        ...
        if (!profile) {
          ...
          const newProfile = {
            ...
            // Every new account gets Journey free for one month (never NULL).
            // The expiry is enforced at read-time (below) and by the daily cron;
            // when it lapses the user drops to the free 'parents' base.
            user_type: 'journey',
      ...
      let activeTier = rawTier && TIER_MIGRATION[rawTier] ? TIER_MIGRATION[rawTier] : rawTier

      // Journey free-month enforcement: if the free month has lapsed, drop to the
      // free base at read-time. The daily cron covers users who never return.
      const journeyExpiresAt = profile.journey_expires_at || null
      if (activeTier === 'journey' && journeyExpiresAt && new Date(journeyExpiresAt).getTime() < Date.now()) {
        activeTier = 'parents'
        supabase.from('users_profile')
          .update({ selected_tier: 'parents', user_type: 'parents' })
          .eq('id', authUser.id)
          .then(() => console.log('[auth] journey free month expired -> downgraded to parents'))
          .catch(err => console.error('[auth] expiry downgrade failed:', err.message))
      }
