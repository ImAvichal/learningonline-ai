...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED' || event === 'USER_UPDATED') {
        ...
        // onAuthStateChange runs while holding the auth lock; awaiting loadUser
        // (which makes DB queries) here blocks getSession() in init() and causes
        // the infinite-spinner / "signup does nothing" hangs. Defer to the next
        // tick so the lock is released before loadUser runs.
  ...
  const loadUser = async (authUser) => {
    ...
    try {
      ...
      if (!profile) {
        ...
        if (!profile) {
          ...
          const newProfile = {
            ...
            // Baseline free tier — never NULL. A brand-new account starts on the
      ...
      // STEP 4: BASELINE user — set this IMMEDIATELY once the profile is
      // known, before touching progress/scores/purchases. This is the fix
      // for a real bug: previously, a single failure anywhere in the
      // enrichment queries below (progress/scores/purchases) would throw,
      // hit the outer catch, and leave `user` at null FOREVER — even though
      // the profile itself (all Nav/header/tier-gating actually needs) had
      // already been fetched successfully. The person's session was fully
      // valid the whole time; only the React state never reflected it. Now
      // the header always reflects reality as soon as the profile is known,
      // and enrichment failures degrade gracefully instead of hiding login.
      setUser({
        ...profile,
        name: profile.full_name,
        tier: profile.selected_tier,
        journeyExpiresAt: profile.journey_expires_at || null,
        progress: {},
        scores: {},
      })
      console.log('[auth] ✓ baseline user set (tier:', profile.selected_tier, ') — enriching…')

      // STEP 5: Enrichment — progress, scores, purchase-reconciled tier. Own
      // try/catch: if ANY of this fails, the baseline user set above stays
      // intact and visible; we simply don't get the extra detail this time.
      try {
        ...
        // Sync profile tier if purchase exists but profile not yet updated.
        ...
        console.error('[auth] loadUser enrichment failed (non-fatal — baseline user stays set):', enrichErr)
      }
    } catch (e) {
      console.error('[auth] loadUser fatal error (profile fetch/self-heal failed):', e)
    }
  }
  ...
  const logout = async () => {
    // Clear the LOCAL session (localStorage) without depending on the global
    // network revoke, which can hang and leave the session "in play". Then do a
    // hard reload to home so all auth state resets from a clean mount — this
    // avoids the auth-guard redirect flash and any stale in-memory state.
    try {
  ...
  const updateUser = async (updates) => {
    ...
    if (user?.id && !user?.isDevUser) {
      ...
      if (updates.journeyExpiresAt !== undefined) dbUpdates.journey_expires_at = updates.journeyExpiresAt
