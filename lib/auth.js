...
export function AuthProvider({ children }) {
  ...
  const loadUser = async (authUser) => {
    try {
      ...
      const activeTier = latestPurchase?.tier || profile.selected_tier || null
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, signup, logout, loginWithGoogle, updateUser, markLessonComplete, saveScore, trackDownload, supabase }}>
      {children}
...
export function useProgress() {
  ...
  const getTotalProgress = () => {
    ...
    const accessible = ALL_LESSONS.filter(l => {
      ...
      return order.indexOf(l.tier) <= order.indexOf(user.tier || 'individual')
    })
...
export function withAuth(Component) {
  return function Protected(props) {
    ...
    useEffect(() => {
      ...
      // Authenticated but no tier — send to pricing, not login
