...
import { Logo, Input, Spinner } from '../components/ui'
...
export default function Login() {
  ...
  return (
    <>
      <Head><title>Sign In — Flowcore AI Academy</title></Head>
      <div className="min-h-screen flex">
        <div className="hidden lg:flex flex-col justify-between w-1/2 bg-navy-mid border-r border-white/5 p-12">
          <Logo />
          <div>
            <div className="text-5xl mb-8">🎯</div>
            <h2 className="font-display font-bold text-3xl mb-4 leading-tight">Welcome back.<br/>Your progress<br/>is waiting.</h2>
            <p className="text-muted leading-relaxed max-w-sm">Pick up exactly where you left off. All your lessons, templates, and deliverables are saved.</p>
          </div>
          <p className="text-xs text-muted">learningonline.ai · Secure login</p>
        </div>
        <div className="flex-1 flex items-center justify-center px-6 py-12">
          <div className="w-full max-w-sm">
            <div className="lg:hidden mb-8"><Logo /></div>
            <h1 className="font-display font-bold text-2xl mb-2">Sign in</h1>
            <p className="text-muted text-sm mb-8">No account? <Link href="/signup" className="text-blue-bright hover:underline">Sign up</Link></p>

            {error && <div className="mb-5 p-4 rounded-lg bg-red-500/10 border border-red-500/25 text-red-400 text-sm">{error}</div>}
