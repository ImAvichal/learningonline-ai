...
import { getTemplatesForTier } from '../../data/templates'
...
export default function TemplatesPage() {
  ...
  const byTier = {
    individual: templates.filter(t => t.tier === 'individual'),
    smb:        templates.filter(t => t.tier === 'smb'),
    enterprise: templates.filter(t => t.tier === 'enterprise'),
  }
    ...
    <div className="p-8 lg:p-10">
      <div className="mb-8">
        ...
        <p className="text-muted text-sm">
          {templates.length} templates included with your <strong className="text-white capitalize">{tier}</strong> tier.
      ...
      <TierSection tierId="smb"        label="SMB tier and above"       items={byTier.smb} />
      ...
      {tier !== 'enterprise' && (
        <div className="mt-4 p-8 border border-dashed border-white/10 rounded-xl text-center">
          ...
          <h4 className="font-display font-bold text-base mb-2">
            {tier === 'individual' ? 'SMB & Enterprise templates locked' : 'Enterprise templates locked'}
          </h4>
          <p className="text-sm text-muted max-w-md mx-auto mb-4">
            {tier === 'individual'
              ? 'Upgrade to SMB or Enterprise to unlock industry playbooks, implementation guides, and the full enterprise operating model template set.'
