...
// Import from here everywhere. Never duplicate.
...
// ── Brand constants ────────────────────────────────────────────────────────────
export const BRAND = {
  name:      'Flowcore AI Academy',
  shortName: 'Flowcore AI',
  domain:    'learningonline.ai',
  parent:    'Praise Consulting',
  parentUrl: 'https://praiseconsulting.com.au',
...
export function Logo({ size = 'md', linked = true }) {
  ...
  const el = (
    <span className={`font-display font-black tracking-tight ${sizes[size]}`}>
      Flowcore<span className="text-blue">AI</span>
      <span className="text-muted font-normal text-[10px] tracking-[2px] ml-1.5 hidden sm:inline">ACADEMY</span>
    </span>
  )
        ...
        <div className="hidden md:flex items-center gap-3">
          {user?.tier ? (
            <>
              ...
              <button onClick={logout} className="text-sm text-muted hover:text-white">Sign Out</button>
            </>
          ) : (
            <>
...
const NavLink = ({ href, children }) => (
  <Link href={href} className="text-sm text-muted hover:text-white transition-colors">{children}</Link>
)
const MobileLink = ({ href, children, onClick, bold }) => (
  <Link href={href} onClick={onClick}
    className={`block text-sm ${bold ? 'font-display font-bold text-blue' : 'text-muted'}`}>
    {children}
  </Link>
)
...
export function TierBadge({ tier, label, className = '' }) {
  const cfg = {
    ...
    smb:        { cls:'tier-smb',        text: label || 'SMB' },
