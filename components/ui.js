  ...
  return (
    <aside className="hidden lg:flex flex-col w-56 bg-white border-r border-gray-200 fixed inset-y-0 left-0 z-30">
      <div className="px-5 pt-6 pb-4 border-b border-gray-200">
        <Logo size="sm" />
        <div className="text-[9px] text-gray-500 tracking-[2px] uppercase mt-1">learningonline.ai</div>
      </div>
      ...
      <div className="p-4 border-t border-gray-200">
        ...
        <div className="flex items-center gap-2.5 mb-3">
          <div className="w-8 h-8 rounded-full bg-blue/15 border border-blue/30 flex items-center justify-center text-xs font-display font-bold text-blue flex-shrink-0">
            {initial}
          </div>
          <div className="min-w-0">
            <div className="text-xs font-display font-bold text-gray-900 truncate">{user?.name}</div>
            <div className="text-[10px] text-gray-500 truncate">{user?.email}</div>
          </div>
        </div>
        <button onClick={logout} className="text-xs text-gray-500 hover:text-gray-900 w-full text-left">Sign Out →</button>
      </div>
    </aside>
  )
}
...
export function Card({ children, className = '', glow = false, hover = false, light = false, style }) {
  // Two variants:
...
export function ProgressBar({ value, className = '', color = 'blue', light = false }) {
  const c = { blue: 'bg-blue', success: 'bg-success', amber: 'bg-amber-400', purple: 'bg-purple-400' }
  const track = light ? 'bg-gray-200' : 'bg-white/5'
  return (
    <div className={`h-1.5 ${track} rounded-full overflow-hidden ${className}`}>
...
export function TierBadge({ tier, label, className = '' }) {
  const cfg = {
    parents:    { cls: 'tier-parents',    text: label || 'Parents & Caregivers' },
