// pages/parents.js — Parents & Caregivers (Free Module)
// URL kept as /parents for backwards compatibility
...
const LESSONS = [
  {
    id: 'p-l1',
    number: 1,
    title: 'What AI Actually Is — For Parents & Caregivers',
    duration: '8 min',
    icon: '🧠',
...
export default function ParentsModule() {
  ...
  // Initial active lesson: URL ?lesson=<id> → first incomplete → 0
  const [activeLesson, setActiveLesson] = useState(() => {
    if (typeof window === 'undefined') return 0
    const params = new URLSearchParams(window.location.search)
    const urlLessonId = params.get('lesson')
    if (urlLessonId) {
      const idx = LESSONS.findIndex(l => l.id === urlLessonId)
      if (idx >= 0) return idx
    }
    if (user?.progress) {
      const firstIncomplete = LESSONS.findIndex(l => !user.progress[l.id])
      if (firstIncomplete >= 0) return firstIncomplete
    }
    return 0
  })
  // Initialise from saved progress (user.progress is populated by loadUser() in auth.js)
  // Local state mirrors this so we can do optimistic UI updates on Mark Complete.
  const [completed, setCompleted] = useState(() => {
    const initial = {}
    if (user?.progress) {
      LESSONS.forEach(l => { if (user.progress[l.id]) initial[l.id] = true })
    }
    return initial
  })

  // Sync local state when user.progress changes (e.g. after loadUser refreshes)
  useEffect(() => {
    if (user?.progress) {
      const next = {}
      LESSONS.forEach(l => { if (user.progress[l.id]) next[l.id] = true })
      setCompleted(next)
    }
  }, [user?.progress])
  ...
  const markDone = async () => {
    // 1. Optimistic UI — show completion immediately
    setCompleted(prev => ({ ...prev, [lesson.id]: true }))

    // 2. Persist to DB via auth context (writes to course_progress.upsert)
    // moduleId is 'parents' since all lessons are part of the Parents module
    if (markLessonComplete) {
      try {
        await markLessonComplete(lesson.id, 'parents')
      } catch (err) {
        console.error('[parents] Failed to save progress:', err.message)
        // Don't block the user — the UI advance still happens
      }
    }

    // 3. Claim the 'parents' tier on first lesson completion (idempotent)
    // This is what makes /dashboard recognise the user as a free Parents user.
    if (claimParentsTier && !user?.tier) {
      try { await claimParentsTier() } catch (err) {
        console.error('[parents] Failed to claim tier:', err.message)
      }
    }

    // 4. Either advance to next lesson, or — if final lesson — celebrate.
      ...
      <div className="pt-20 pb-20 min-h-screen">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="mb-8 pt-4">
            ...
            <h1 className="font-display font-black text-3xl sm:text-4xl tracking-tight mb-2 break-words">Parents & Caregivers</h1>
              ...
              {!user?.tier && !user?.isDevUser && (
                <div className="mt-4 p-4 rounded-xl border border-blue/20 bg-blue/5">
                  ...
                  <p className="text-xs text-muted leading-relaxed mb-3">
                    14 learning modules on AI strategy, use cases, ROI, and execution — starting at $19.
                  </p>
                  <Link href="/pricing"
