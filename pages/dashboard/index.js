...
function Dashboard() {
  ...
  // ── Switch to course tab AND deep-link to a specific lesson ─────────────
  // CoursePage reads ?lesson=<id> on mount to set the right initial lesson.
    ...
    <>
      ...
      <div className="min-h-screen flex bg-gray-50">
        ...
        <main className="flex-1 lg:ml-56 min-h-screen overflow-auto">
          {tab === 'home'      && <HomeTab user={user} totalPct={totalPct} completedMods={completedMods} nextLesson={nextLesson} switchTab={switchTab} goToLesson={goToLesson} getModuleProgress={getModuleProgress} isCompleted={isCompleted} />}
...
function HomeTab({ user, totalPct, completedMods, nextLesson, switchTab, goToLesson, getModuleProgress, isCompleted }) {
  return (
    <div className="p-8 lg:p-10 max-w-4xl">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1 flex-wrap">
          <h1 className="font-display font-black text-2xl sm:text-3xl break-words">Welcome back, {user.name?.split(' ')[0] || 'there'} 👋</h1>
          {user.isDevUser && <TierBadge tier="enterprise" label="Dev Access" />}
        </div>
        <p className="text-gray-500">{totalPct === 0 ? "Let's get you started — one focused lesson at a time." : totalPct === 100 ? "You've completed the pathway. Revisit anything any time." : "You're making real progress. Pick up where you left off."}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <Card light key={i} className="p-5">
            <div className={`font-display font-black text-3xl mb-1 ${k.color}`}>{k.val}</div>
            <div className="text-xs font-display font-bold text-gray-900">{k.label}</div>
      ...
      <Card light className="p-5 mb-5">
        ...
        <ProgressBar light value={totalPct} className="mb-2" />
        <p className="text-xs text-gray-500">
          {totalPct === 0   && 'Start with Module 1 — your first 15 minutes set the tone.'}
          {totalPct > 0 && totalPct < 100 && 'Great progress — you\'re building real AI confidence.'}
          {totalPct === 100 && '🎉 Pathway complete. You\'ve earned it.'}
        </p>
      </Card>

      {/* Milestone reinforcement — shows once you cross 25/50/75% */}
      {totalPct >= 25 && totalPct < 100 && (
        <div className="bg-success/5 border border-success/20 rounded-xl px-5 py-3 mb-5 flex items-center gap-3">
          <span className="text-lg" aria-hidden="true">
            {totalPct >= 75 ? '🎯' : totalPct >= 50 ? '🌟' : '✨'}
          </span>
          <p className="text-sm text-gray-500 leading-relaxed">
            {totalPct >= 75 && "You're in the final stretch — most of the path is behind you."}
            {totalPct >= 50 && totalPct < 75 && "You're more than halfway through. Real AI confidence is building."}
            {totalPct >= 25 && totalPct < 50 && "Solid start — you're building a foundation that compounds."}
          </p>
        </div>
      )}
        ...
        <div className="bg-blue/5 border border-blue/30 rounded-xl p-5 mb-8 flex items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="text-xs font-display font-bold text-blue-bright uppercase tracking-wider mb-1">
              {totalPct === 0 ? 'Start Here' : `Welcome back — ready to continue?`}
            </div>
            <div className="font-display font-bold text-base text-gray-900 mb-0.5 truncate">{nextLesson.title}</div>
            <div className="text-sm text-gray-500 truncate">Module {nextLesson.moduleNumber}: {nextLesson.moduleTitle}</div>
          </div>
        ...
        <div className="mb-8 p-6 rounded-xl bg-purple-400/5 border border-purple-400/20">
          <div className="flex items-start gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Operating model, data readiness program, governance frameworks, command centre design — all included.</p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[['⚙️','Operating Model'],['🗄️','Data Readiness'],['⚖️','Governance'],['🖥️','Command Centre']].map(([icon,label]) => (
              <button key={label} onClick={() => switchTab('course')}
                className="p-3 bg-gray-50 border border-gray-200 rounded-lg text-left hover:border-purple-400/30 transition-colors">
                <div className="text-xl mb-1">{icon}</div>
                <div className="text-xs font-display font-bold text-gray-900">{label}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Module list */}
      <div className="mb-6">
        <div className="text-xs font-display font-bold text-gray-500 uppercase tracking-widest mb-4">All Modules</div>
                ...
                <Card light hover className={`p-4 ${isIP ? 'border-blue/25 bg-blue/[0.03]' : ''}`}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="font-display font-bold text-sm text-gray-900 truncate">{mod.title}</div>
                      <ProgressBar light value={prog} className="mt-2" />
                    </div>
                    <svg className="text-gray-500 flex-shrink-0 w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg>
                  </div>
                </Card>
              </button>
            )
