...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    if (res.success) {
      ...
      // Show the actual error from auth — distinguishes "wrong password" from "timeout"
            ...
            <p className="text-muted text-sm mb-8">
              ...
              <Link 
                  href={router.query.redirect 
                    ...
                    : '/signup'} 
                  className="text-blue-bright hover:underline">Create one free</Link>
            </p>

            {error && (
              <div className="mb-5 p-4 rounded-lg bg-red-500/10 border border-red-500/25 text-red-400 text-sm break-words">{error}</div>
            )}
