...
const TERMS = [
  ...
  },    { term: 'Batch Processing', category: 'Infrastructure', definition: 'Grouping multiple AI requests and processing them together in a single run.', explanation: 'Like a bus route instead of individual taxis. Most AI providers offer 30-50% discounts for batch processing.', example: 'A company classifies 10,000 emails overnight in batch instead of processing each one live — saving 40% on AI costs.', importance: 'One of the simplest and highest-impact cost reduction strategies for AI operations.' },
    { term: 'Context Length', category: 'Models', definition: 'The maximum amount of text an AI model can read and remember in a single request, measured in tokens.', explanation: 'The model\'s working memory. Longer documents cost more because the model holds all that text in active memory.', example: 'GPT-4o has a 128K token context window. Sending a 100-page document costs far more than sending a 5-page summary.', importance: 'Sending less text via RAG or chunking is almost always better than sending entire documents.' },
    { term: 'KV Cache', category: 'Infrastructure', definition: 'Key-Value cache — the internal memory an AI model builds during a conversation.', explanation: 'The longer the conversation, the larger the cache, and the more expensive each response becomes.', example: 'A 50-message thread costs more per response than a fresh 2-message exchange.', importance: 'Starting new conversations for new topics is cheaper than continuing one long thread.' },
    { term: 'Model Routing', category: 'Architecture', definition: 'Directing different AI tasks to different models based on complexity and cost.', explanation: 'Simple tasks go to cheap models; complex tasks go to powerful ones.', example: 'Spam detection routes to a tiny model, response drafting to mid-tier, legal flagging to frontier.', importance: 'Model routing typically reduces AI costs by 40-70% versus using one frontier model for everything.' },
    { term: 'Mixture of Experts', category: 'Models', definition: 'A model architecture where only a subset of the model activates for each request.', explanation: 'Like a hospital where only the relevant specialists are consulted — large capacity but efficient per-request.', example: 'Google Gemini uses Mixture of Experts for strong performance at competitive cost.', importance: 'MoE models offer large-model quality at closer to small-model cost per request.' },
    { term: 'Inference', category: 'Infrastructure', definition: 'The process of an AI model generating a response. Every AI use runs inference.', explanation: 'Training is how a model learns (once). Inference is how it answers (every request). API bills are inference costs.', example: 'When you ask ChatGPT a question, GPU inference generates the response — that is what you pay for.', importance: 'Every AI response costs inference compute. Minimise unnecessary calls through caching and batching.' },
    { term: 'Caching', category: 'Architecture', definition: 'Storing AI responses so identical future requests are answered from stored results.', explanation: 'If 100 customers ask the same question, caching processes it once and serves the stored answer 99 times.', example: 'An FAQ chatbot caches responses for the top 200 questions. 55% served from cache, halving API costs.', importance: 'The simplest way to reduce AI costs for applications with repetitive queries.' },
    { term: 'Background Processing', category: 'Infrastructure', definition: 'Running AI tasks without real-time urgency, allowing batch processing at lower cost.', explanation: 'Daily summaries, weekly classifications, nightly processing can run in background mode at 30-50% lower cost.', example: 'A logistics company processes feedback overnight in batch instead of real-time, reducing AI costs by 45%.', importance: 'Identifying background-eligible tasks saves 30-50% with zero implementation cost.' },
