...
export default function Glossary() {
  ...
  return (
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="mt-12 p-8 rounded-2xl border border-blue/20 bg-blue/5 text-center">
            ...
            <p className="text-muted text-sm mb-5 max-w-md mx-auto">
              The Individual Upskill program applies every term in this glossary to real business scenarios — with exercises, templates, and Q&A.
            </p>
    ...
    { term: 'Batch Processing', category: 'Infrastructure', definition: 'Grouping multiple AI requests and processing them together in a single run rather than one at a time.', explanation: 'Like a bus route instead of individual taxis — each passenger (request) is cheaper when they share the journey. Most AI providers offer 30–50% discounts for batch processing.', example: 'A company classifies 10,000 customer emails overnight in a single batch run instead of processing each one the moment it arrives — saving 40% on AI costs.', importance: 'Batch processing is one of the simplest and highest-impact cost reduction strategies for any AI operation that does not require real-time responses.' },
    { term: 'Latency', category: 'Infrastructure', definition: 'The time delay between sending a request to an AI model and receiving the response.', explanation: 'Lower latency means faster responses. In AI, faster responses typically cost more because they require dedicated infrastructure running at full capacity.', example: 'A customer chatbot needs low latency (under 2 seconds) for good user experience. A nightly report generator can tolerate high latency (minutes) and run cheaper.', importance: 'Understanding latency requirements helps you choose the right processing mode — real-time for user-facing tasks, background for everything else — and control costs accordingly.' },
    { term: 'Context Length', category: 'Models', definition: 'The maximum amount of text an AI model can read and remember in a single request, measured in tokens.', explanation: 'Think of it as the model\'s working memory. A longer context window means the model can process longer documents, but the cost increases with length because the model must hold all that text in active memory.', example: 'GPT-4o has a 128K token context window — roughly a 96,000-word novel. But sending a 100-page document costs significantly more than sending a 5-page summary.', importance: 'Context length directly affects cost and performance. Sending less text (via RAG or chunking) is almost always better than sending entire documents.' },
    { term: 'KV Cache', category: 'Infrastructure', definition: 'Key-Value cache — the internal memory structure an AI model builds during a conversation to remember what has been said.', explanation: 'Every time you continue a conversation, the model rebuilds its memory of everything said so far. The longer the conversation, the larger the KV cache, and the more expensive each new response becomes.', example: 'A 50-message conversation thread costs more per response than a fresh 2-message exchange because the model must process the entire history before generating each reply.', importance: 'This is why starting new conversations for new topics is cheaper than continuing one long thread — and why long-document processing benefits from chunking strategies.' },
    { term: 'Model Routing', category: 'Architecture', definition: 'The practice of automatically directing different AI tasks to different models based on complexity, cost, and accuracy requirements.', explanation: 'Instead of using one expensive model for everything, a routing layer decides which model handles each request. Simple classification goes to a cheap model; complex reasoning goes to a powerful one.', example: 'An email system routes spam detection to a tiny model (cost: $0.0001/email), customer response drafting to a mid-tier model ($0.005/email), and legal review flagging to a frontier model ($0.02/email).', importance: 'Model routing typically reduces AI operating costs by 40–70% compared to using a single frontier model for all tasks.' },
    { term: 'Mixture of Experts', category: 'Models', definition: 'A model architecture where only a subset of the model\'s parameters activate for each request, rather than the entire model.', explanation: 'Imagine a hospital with 100 specialists. For any patient, only 3–5 relevant specialists are consulted — not all 100. MoE models work the same way: large overall capacity, but efficient per-request because only the relevant expert components activate.', example: 'Google\'s Gemini uses a Mixture of Experts architecture, which is how it delivers strong performance while keeping inference costs competitive.', importance: 'MoE models offer large-model quality at closer to small-model cost per request — an important factor when evaluating model pricing.' },
    { term: 'Inference', category: 'Infrastructure', definition: 'The process of an AI model generating a response to a request. Every time you use AI, you are running inference.', explanation: 'Training is how a model learns (done once, very expensive). Inference is how a model answers (done every request, cost scales with usage). When you pay for AI API calls, you are paying for inference compute.', example: 'When you ask ChatGPT a question, the inference process converts your tokens into a response using GPU compute — and that compute is what appears on your bill.', importance: 'Understanding that every AI response costs inference compute helps you design systems that minimise unnecessary calls through caching, batching, and right-sizing.' },
    { term: 'Caching', category: 'Architecture', definition: 'Storing AI responses so that identical or very similar future requests can be answered from the stored response instead of making a new API call.', explanation: 'If 100 customers ask the same question, caching means the AI processes it once and serves the stored answer 99 times — eliminating 99% of the cost for that query.', example: 'An e-commerce FAQ chatbot caches responses for the top 200 most common questions. 55% of all queries are served from cache — cutting monthly API costs by more than half.', importance: 'Caching is the simplest way to reduce AI costs for any application with repetitive queries. Implementation is straightforward and the ROI is immediate.' },
    { term: 'Background Processing', category: 'Infrastructure', definition: 'Running AI tasks during off-peak times or without real-time urgency, allowing for batch processing and lower costs.', explanation: 'Not every AI task needs to happen instantly. Tasks like daily report summaries, weekly data classification, or nightly email processing can run in background mode at significantly lower cost.', example: 'A logistics company processes delivery feedback overnight in batch mode instead of real-time, reducing AI costs by 45% with no impact on operations since the analysis is reviewed the next morning.', importance: 'Identifying which tasks can run in background mode is one of the most effective cost management strategies — it costs nothing to implement and saves 30–50% on affected use cases.' },
