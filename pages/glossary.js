/
    ...
    { term: 'Context Length', category: 'Models', definition: 'The maximum amount of text an AI model can read and remember in a single request, measured in tokens.', explanation: 'Think of it as the model\'s working memory. A longer context window means the model can process longer documents, but the cost increases with length.', example: 'GPT-4o has a 128K token context window — roughly a 96,000-word novel. But sending a 100-page document costs significantly more than sending a 5-page summary.', importance: 'Context length directly affects cost and performance. Sending less text (via RAG or chunking) is almost always better than sending entire documents.' },
    { term: 'KV Cache', category: 'Infrastructure', definition: 'Key-Value cache — the internal memory structure an AI model builds during a conversation to remember what has been said.', explanation: 'Every time you continue a conversation, the model rebuilds its memory of everything said so far. The longer the conversation, the larger the cache, and the more expensive each response becomes.', example: 'A 50-message conversation costs more per response than a fresh 2-message exchange because the model must process the entire history before generating each reply.', importance: 'This is why starting new conversations for new topics is cheaper than continuing one long thread.' },
    { term: 'Model Routing', category: 'Architecture', definition: 'Automatically directing different AI tasks to different models based on complexity, cost, and accuracy requirements.', explanation: 'Instead of using one expensive model for everything, a routing layer decides which model handles each request. Simple tasks go to cheap models; complex tasks go to powerful ones.', example: 'An email system routes spam detection to a tiny model ($0.0001/email), response drafting to a mid-tier model ($0.005/email), and legal flagging to a frontier model ($0.02/email).', importance: 'Model routing typically reduces AI operating costs by 40–70% compared to using a single frontier model for all tasks.' },
    { term: 'Mixture of Experts', category: 'Models', definition: 'A model architecture where only a subset of the model activates for each request, rather than the entire model.', explanation: 'Like a hospital with 100 specialists — for any patient, only 3–5 relevant specialists are consulted. MoE models work the same way: large capacity, but efficient per-request.', example: 'Google Gemini uses a Mixture of Experts architecture, delivering strong performance while keeping inference costs competitive.', importance: 'MoE models offer large-model quality at closer to small-model cost per request — important when evaluating model pricing.' },
    { term: 'Inference', category: 'Infrastructure', definition: 'The process of an AI model generating a response to a request. Every time you use AI, you are running inference.', explanation: 'Training is how a model learns (done once, very expensive). Inference is how a model answers (done every request, cost scales with usage). API bills are inference costs.', example: 'When you ask ChatGPT a question, the inference process converts your tokens into a response using GPU compute — that compute is what appears on your bill.', importance: 'Understanding that every AI response costs inference compute helps you design systems that minimise unnecessary calls.' },
    { term: 'Caching', category: 'Architecture', definition: 'Storing AI responses so that identical future requests can be answered from the stored response instead of making a new API call.', explanation: 'If 100 customers ask the same question, caching means the AI processes it once and serves the stored answer 99 times — eliminating 99% of the cost for that query.', example: 'An e-commerce FAQ chatbot caches responses for the top 200 questions. 55% of queries are served from cache — cutting monthly API costs by more than half.', importance: 'Caching is the simplest way to reduce AI costs for any application with repetitive queries.' },
    { term: 'Background Processing', category: 'Infrastructure', definition: 'Running AI tasks during off-peak times or without real-time urgency, allowing for batch processing and lower costs.', explanation: 'Not every AI task needs to happen instantly. Daily summaries, weekly classifications, or nightly processing can run in background mode at significantly lower cost.', example: 'A logistics company processes delivery feedback overnight in batch mode instead of real-time, reducing AI costs by 45%.', importance: 'Identifying which tasks can run in background mode saves 30–50% on affected use cases with zero implementation cost.' },/ pages/glossary.js — Le On AI Glossary
